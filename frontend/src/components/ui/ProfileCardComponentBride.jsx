...
const ANIMATION_CONFIG = { SMOOTH_DURATION: 600, INITIAL_DURATION: 800, INITIAL_X_OFFSET: 0, INITIAL_Y_OFFSET: 0, DEVICE_BETA_OFFSET: 20 };
...
const ProfileCardComponent = ({
    ...
    const lastMoveTime = useRef(0); // 🚀 ADDED - for throttling
    ...
    const animationHandlers = useMemo(() => {
        ...
        const updateCardTransform = (offsetX, offsetY, card, wrap) => {
            // 🚀 ADDED THROTTLE - only update every 16ms (60fps)
            const now = Date.now();
        ...
        // ✅ KEEP ORIGINAL createSmoothAnimation
    ...
    useEffect(() => {
        observeElements(".pc-card-wrapper_", (el) => {
        ...
        const initialX = wrap.clientWidth / 4;
        const initialY = wrap.clientHeight / 4;
    ...
    // ✅ KEEP ORIGINAL cardStyle and return JSX
    ...
    return (
        <div ref={wrapRef} className={`pc-card-wrapper_ ${className}`.trim()} style={cardStyle}>
            <section ref={cardRef} className="pc-card">
                <div className="pc-inside">
                    <div className="pc-shine" />
                    <div className="pc-glare" />
                    <div className="pc-content pc-avatar-content_k">
                        <img className="avatar_k" src={avatarUrl} alt={`${name || "User"} avatar`} loading="lazy" onError={(e) => { const target = e.target; target.style.display = "none"; }} />
                        {showUserInfo && (
                            <div className="pc-user-info">
                                <div className="pc-user-details">
                                    <div className="pc-mini-avatar">
                                        <img src={miniAvatarUrl || avatarUrl} alt={`${name || "User"} mini avatar`} loading="lazy" onError={(e) => { const target = e.target; target.style.opacity = "0.5"; target.src = avatarUrl; }} />
                                    </div>
                                    <div className="pc-user-text">
                                        <div className="pc-handle">{handle ? `@${handle}` : ""}</div>
...
const ProfileCardbride = React.memo(ProfileCardComponent);
export default ProfileCardbride;